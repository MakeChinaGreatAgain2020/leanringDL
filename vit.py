...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        """dim=768,heads=12,dim_head=3072"""
        super().__init__()
...
class ViT(nn.Module):
    ...
    def forward(self, img):
        ...
        x = torch.cat((cls_tokens, x), dim=1)#x shape = [16, 197, 768]
        x += self.pos_embedding[:, :(n + 1)]#第1、3维度保留全部，第2维度保留前197个元素
        
        x = self.dropout(x)

        x = self.transformer(x)#x shape = [16, 197, 768]
        
        x = x.mean(dim = 1) if self.pool == 'mean' else x[:, 0]#x shape = [16, 768]
        
        x = self.to_latent(x)#x shape = [16, 768]
        x = self.mlp_head(x)#x shape = [16, 10]
        #print("shape= ", x.shape)
        return x
...
if __name__ == "__main__":
    ...
    img = torch.randn(16, 3, 224, 224)  # 16张224x224的RGB图像
    preds = model(img)
